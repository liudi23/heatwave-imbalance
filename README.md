...
## Status
Phase 1, in progress. Done: scope, reuse scaffolding, **all four physical-driver fetchers** (demand, generation-by-fuel, interconnectors, PV_Live solar), the anchor figure, a first-pass H1–H5 driver attribution (proxy mode), and a read-only Streamlit results app. The decomposition auto-upgrades to full mode when the MW data is present — swapping the irradiance proxy for the **absolute solar drop-off (H2)** and adding **net-demand ramp (H1)** and **interconnector imports (H5)**. Next: run the four fetchers over the heatwave windows to populate full mode, then write the short public writeup.
